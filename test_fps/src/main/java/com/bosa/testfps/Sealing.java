...
-----END PRIVATE KEY-----
	...
	static void handleJsonSealing(HttpExchange httpExch, Map<String, String> queryParams) throws Exception {
		...
		reply = postJson(sealingSignUrl + "/signing/getDataToSign", payLoad);
		...
		if (cred != null) {
			...
			payLoad = "{ \"signatureLevel\":\"RAW\", \"digest\":\"" + hashToSign + "\", \"digestAlgorithm\":\"" + digestAlgo + "\", \"signer\":{\"enterpriseNumber\": " + oai.enterpriseNumber + ",\"certificateAlias\":\"" + oai.signerId + "\"}}";
		...
		reply = postJson(sealingSignUrl + "/signing/signDocument", payLoad);
